...
class Attack(object):
    ...
    def __init__(self, attack, model_name, epsilon, targeted, random_start, norm, loss, device=None):
        ...
        self.attack = attack
        self.model = _PRELOADED_MODEL if _PRELOADED_MODEL is not None else self.load_model(model_name)
    ...
    def load_model(self, model_name):
        ...
        def load_single_model(model_name):
            # Support DINOv3 family via the shared loader
            if model_name.lower().startswith('dinov3'):
                from .utils import load_single_model as _utils_load
                return _utils_load(model_name, dataset_name='ImageNet',
    ...
    def forward(self, data, label, **kwargs):
        ...
        # with torch.no_grad():
        #     l_inf_real = delta.abs().max().item()
        #     l_2_real = delta.view(delta.size(0), -1).norm(p=2, dim=1).mean().item()
        #     data_min = data.min().item()
        #     data_max = data.max().item()
        #     print(f"\n[FGSM Debug] Input Data Range: [{data_min:.4f}, {data_max:.4f}]")
        #     print(f"[FGSM Debug] Raw Delta L_inf: {l_inf_real:.6f} (Expected: <= 0.0627)")
        #     print(f"[FGSM Debug] Raw Delta L_2: {l_2_real:.4f} (Expected: <= 24.34)")
